...
int io_rspi_transfer(unsigned char *write_data, unsigned char *read_data, int data_sz)
{
    unsigned short tmp;
	...
	/* Writes the transmit data to the data register */
	if(write_data != (unsigned char *)0){

	    tmp = (unsigned short)*write_data++;
	}
	else{
	    tmp = 0x00ff; /* Dummy write data */
	}


	RSPI0.SPDR.WORD.L = 0x0;
	RSPI0.SPDR.WORD.H = 0x00ff & tmp;

	RSPI0.SPSR.BIT.SPTEF = 0;
	/* Clears the bit to 0 to transmit data */
	/* Waits until the reception is completed */
	while((RSPI0.SPSR.BYTE & 0x81) == 0x00){
	    /* Waits until the receive buffer is full or an overrun error occurs */
	}
	...
	if(RSPI0.SPSR.BIT.OVRF == 1){
	    ...
	    SFL_CHIP_SELECT_CLR;
	    return -1; /* Overrun error occurred */
	}

	/* Reads the received data */
	tmp = RSPI0.SPDR.WORD.H;
	if(read_data != (unsigned char *)0){
	    *read_data++ = (unsigned char)tmp;
	}
...
int read_flash_id(uint8_t idbuf[3])
{
    unsigned char in[10];
    unsigned short tmp;
	...
	RSPI0.SPDR.WORD.L = 0x0;
	RSPI0.SPDR.WORD.H = 0x00ff & SPI_FLASH_INS_RDID;
	...
	RSPI0.SPDR.WORD.H = 0x00ff;
	while (RSPI0.SPSR.BIT.IDLNF) // wait for transfer to complete
	    ;
	(void)RSPI0.SPDR.WORD.L;
	idbuf[0] = RSPI0.SPDR.WORD.H;
	...
	while (RSPI0.SPSR.BIT.IDLNF) // wait for transfer to complete
	    ;
	(void)RSPI0.SPDR.WORD.L;
	idbuf[1] = RSPI0.SPDR.WORD.H;

	RSPI0.SPDR.WORD.H = 0x00ff;
	while (RSPI0.SPSR.BIT.IDLNF) // wait for transfer to complete
	    ;
	(void)RSPI0.SPDR.WORD.L;
	tmp = idbuf[2] = RSPI0.SPDR.WORD.H;
...
void vRspioRx( void ) __attribute__((naked));

void vRspioRx( void )
{
    rxByte = 1;//RSPI0.SPDR.WORD.H;
}
